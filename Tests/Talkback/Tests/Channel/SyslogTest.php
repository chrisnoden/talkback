...
class SyslogTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testSyslogLongNameException()
    {
        $this->setExpectedException(
            ...
            'Syslog name must be a string, max 30 chars'
        );
    ...
    public function testSyslogSetFacilityExceptions()
    {
        ...
        try {
            ...
            if ($e->getMessage() == 'facility must be a valid LOG facility constant') {
                $fail = false;
            }
        }
        if ($fail) {
    ...
    public function testSyslogSetOptionExceptions()
    {
        ...
        try {
            ...
            if ($e->getMessage(
                ) == 'option must be a valid constant LOG_CONS, LOG_NDELAY, LOG_ODELAY, LOG_PERROR or LOG_PID (default)'
            ) {
                $fail = false;
            }
        }
        ...
        if ($fail) {
            $this->fail("An expected exception has not been raised when testing wrong constant");
        }
    }
